...
export default function ApiKeysPage() {
    ...
    const handleCreateKey = async () => {
        ...
        try {
            const token = localStorage.getItem("token");
            // Assuming we need project_id, but API might handle it or default to user default
            // For now, let's try creating a key without project_id if API supports it, or error out
            // Actually, backend requires project_id. Let's list projects first or mock it for now.
            // Simplified: asking user for project ID is bad UX. 
            // Better: Auto-select first project? 

            // For this specific 'landing' dashboard, we'll implement a simpler 'Create' that assumes context.
            // BUT, our API requires `projectId`.
            // Let's implement a quick fetch for project ID 0 or allow user to select.

            // Workaround: We'll fetch projects, pick the first one.
            const pRes = await fetch("http://localhost:4000/projects", { headers: { "Authorization": `Bearer ${token}` } });
            const pData = await pRes.json();
            const projectId = pData.data?.[0]?.id;
            ...
            const res = await fetch("http://localhost:4000/api-keys", {
                ...
                body: JSON.stringify({ projectId, name, scopes: ["read", "write"] })
            });
    ...
    const handleRevokeKey = async (id: string) => {
        ...
        try {
            const token = localStorage.getItem("token");
            await fetch(`http://localhost:4000/api-keys/${id}`, {
                method: "DELETE",
                headers: { "Authorization": `Bearer ${token}` }
            });
            fetchKeys();
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between border-b border-white/5 pb-6">
                <div>
                    ...
                    <p className="text-slate-400">Manage access tokens for your applications.</p>
                </div>
            ...
            {loading ? (
                ...
                <div className="text-center py-12 text-slate-500">No API keys found.</div>
            ) : (
